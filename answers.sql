...
SELECT * FROM brands WHERE founded <= (SELECT MIN(founded) FROM brands WHERE founded > (SELECT MIN(founded) FROM brands WHERE founded > (SELECT MIN(founded) FROM brands))) ORDER BY founded;
...
SELECT m.year, m.name, m.model_id, a.name FROM models AS m
LEFT JOIN awards AS a ON (a.winner_id = m.model_id)
WHERE m.year >= 1964;
...
-----
  ...
  SELECT b.name,
         ...
         m.year,
         b.discontinued,
	b.discontinued-year AS years_until_brand_discontinued
  FROM models AS m
...
LEFT JOIN models AS m ON (m.brand_id = b.brand_id)
